	...
	<view class="main-layout">
		...
		<view class="header">
			<view class="header-content">
				...
				<view class="nav-section">
					...
					<text class="nav-item" :class="{active: activeTab === 'circle'}" @click="switchTab('circle')">交流圈</text>
					<text class="nav-item" :class="{active: activeTab === 'about'}" @click="switchTab('about')">关于</text>
		...
		<view class="main-container">
			<view class="container">
				<!-- 首页内容 -->
				<view v-if="activeTab === 'home'" class="content-section">
					<!-- 首页内容组件 -->
				...
				<!-- 交流圈内容 -->
				<view v-if="activeTab === 'circle'" class="content-section">
					<view class="circle-content">
						...
						<view class="circle-grid">
							<view class="circle-card" v-for="(circle, index) in circleList" :key="circle._id" @click="goToCircleDetail(circle._id)">
								...
								<view class="circle-content-area">
				...
				<view v-if="activeTab === 'about'" class="content-section">
					<view class="about-content">
						<view class="about-section">
							<h2>关于我们</h2>
							<p>码客——程序员交流社区致力于为开发者提供一个高质量的技术交流平台。</p>
							<p>在这里，您可以分享技术心得、解决问题、结识志同道合的朋友。</p>
						</view>
					</view>
				</view>
			</view>
...
const circleCategoryDBName = 'circle-categories';
...
export default {
	...
	data() {
		return {
			...
			// 交流圈相关数据
			circleWhere: '', // 交流圈查询条件
			activeCircleCategory: 'all', // 当前选中的交流圈分类ID
	...
	methods: {
		...
		async loadCircleCategories() {
			try {
				console.log('开始加载交流圈分类列表...');
				const result = await uniCloud.callFunction({
					name: 'get-circle-category-list'
				});
				console.log('交流圈分类数据:', result);
				
				// 从云函数获取分类数据
				let fetchedCategories = [];
										
				if(result && result.result && result.result.code === 0 && result.result.data && result.result.data.length > 0) {
					console.log('数据库交流圈分类数据:', result.result.data);
					// 使用从云函数获取的分类数据
					fetchedCategories = result.result.data;
				} else {
					console.log('未找到交流圈分类数据');
				}
										
				// 添加"全部"分类到列表开头
				const allCategory = { _id: 'all', name: '全部' };
				this.circleCategoriesList = [allCategory, ...fetchedCategories];
				console.log('最终交流圈分类列表:', this.circleCategoriesList);
			} catch (e) {
				console.error('加载交流圈分类列表失败', e);
				// 如果加载失败，提供"全部"分类和一些默认分类作为后备
				const allCategory = { _id: 'all', name: '全部' };
				...
				this.circleCategoriesList = [allCategory, ...defaultCategories];
			}
		},
		// 加载交流圈列表
		async loadCircles() {
			try {
				...
				const result = await uniCloud.callFunction({
					...
					params: {
						category_id: this.activeCircleCategory,
						page: this.circleCurrentPage,
				...
				if(result && result.result && result.result.code === 0) {
					...
					// 检查是否还有更多数据
					this.circleHasMore = newCircles.length === this.circlePageSize;
				} else {
		...
		changeCircleCategory(categoryId) {
			...
			this.circleCurrentPage = 1; // 重置到第一页
