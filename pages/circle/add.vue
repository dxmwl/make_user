				...
				<view class="form-container">
					...
					<form @submit="submitForm">
						...
						<view class="form-item">
							...
							<input class="input" v-model="formData.owner_nickname" placeholder="请输入群主昵称" />
						</view>
						...
						<view class="form-item">
							...
							<view class="tag-input-container">
								<input class="input tag-input" v-model="tagInput" @keyup.enter="addTag" placeholder="输入标签后按回车添加" />
								<view class="tag-list">
						...
						<view class="form-item">
							...
							<textarea class="textarea" v-model="formData.remark" placeholder="请输入加入群时的备注信息要求" maxlength="200" :auto-height="true" />
						</view>
	...
	methods: {
		...
		chooseQrCode() {
			uni.chooseImage({
				...
				success: (res) => {
					// 这里应该是上传图片的逻辑，暂时用本地路径
					this.formData.qr_code = res.tempFilePaths[0];
					uni.showToast({
						icon: 'none',
						title: '图片已选择，实际部署时会上传到云端'
					});
				},
				fail: (err) => {
		...
		chooseOwnerQrCode() {
			uni.chooseImage({
				...
				success: (res) => {
					// 这里应该是上传图片的逻辑，暂时用本地路径
					this.formData.owner_qr_code = res.tempFilePaths[0];
					uni.showToast({
						icon: 'none',
						title: '图片已选择，实际部署时会上传到云端'
					});
		...
		async submitForm() {
			...
			if (!this.formData.owner_nickname.trim()) {
				uni.showToast({
					icon: 'none',
					title: '请输入群主昵称'
				});
