		...
		<view class="uni-container">
            ...
            >分类</uni-th>
              ...
              <view class="multi-tip" v-if="item.thumbnail && item.thumbnail.length >= 3">3图</view>
						...
						<uni-td align="center">
							<view class="tags-display" v-if="item.tags && item.tags.length > 0">
								<uni-tag v-for="(tag, index) in item.tags.slice(0, 3)" :text="tag" size="mini" :inverted="true" style="margin-right: 5px;" />
  ...
  mixins: [authMixin],
	data() {
		return {
			...
			collectionList: [
				db.collection(articleDBName).field('is_admin,user_id,category_id,content,title,excerpt,article_status,view_count,like_count,is_sticky,is_essence,comment_status,comment_count,last_comment_user_id,thumbnail,publish_date,publish_ip,last_modify_date,last_modify_ip,tags').getTemp(),
